...
public interface IAnkiConnect
{
    Task StoreMediaFileAsync(string name, string content, CancellationToken token);
    Task AddNoteAsync(string deck, string front, string back, CancellationToken token);
    Task SyncAsync(CancellationToken token);
    Task<List<string>> GetDecksAsync(CancellationToken token);
}

public class AnkiConnect(ILogger<AnkiConnect> logger, HttpClient client) : IAnkiConnect
{
    public Task StoreMediaFileAsync(string name, string content, CancellationToken token)
    {
        logger.LogInformation("Storing media file: {Name}", name);
    ...
    {
        logger.LogInformation("Adding new note to the deck: {Deck}", deck);
        ...
        catch (ApiException e)
        {
            ...
            logger.LogInformation("The note is duplicated");
        }
    }

    public Task SyncAsync(CancellationToken token)
    {
        logger.LogInformation("Requesting synchronization");
        var command = new Dictionary<string, object> { { "action", "sync" }, { "version", 6 } };
        return ExecuteAsync<AnkiConnectResponse>("sync", command, token);
    }

    public async Task<List<string>> GetDecksAsync(CancellationToken token)
    {
        logger.LogInformation("Getting list decks");
        var command = new Dictionary<string, object>
        {
            { "action", "deckNames" },
            { "version", 6 },
